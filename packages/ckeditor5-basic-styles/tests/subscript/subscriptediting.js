...
describe( 'SubscriptEditing', () => {
	...
	it( 'should have pluginName', () => {
		expect( SubscriptEditing.pluginName ).toBe( 'SubscriptEditing' );
	} );

	it( 'should have `isOfficialPlugin` static flag set to `true`', () => {
		expect( SubscriptEditing.isOfficialPlugin ).toBe( true );
	} );

	it( 'should have `isPremiumPlugin` static flag set to `false`', () => {
		expect( SubscriptEditing.isPremiumPlugin ).toBe( false );
	} );

	it( 'should be loaded', () => {
		expect( editor.plugins.get( SubscriptEditing ) ).toBeInstanceOf( SubscriptEditing );
	} );

	it( 'should set proper schema rules', () => {
		expect( model.schema.checkAttribute( [ '$root', '$block', '$text' ], 'subscript' ) ).toBe( true );
		expect( model.schema.checkAttribute( [ '$clipboardHolder', '$text' ], 'subscript' ) ).toBe( true );
	} );

	it( 'should be marked with a formatting property', () => {
		expect( model.schema.getAttributeProperties( 'subscript' ) ).toEqual( expect.objectContaining( {
			isFormatting: true
		} ) );
	} );
	...
	describe( 'command', () => {
		it( 'should register subscript command', () => {
			...
			expect( command ).toHaveProperty( 'attributeKey', 'subscript' );
		} );
	} );
	...
	describe( 'data pipeline conversions', () => {
		it( 'should convert <sub> to sub attribute', () => {
			...
			expect( _getModelData( model, { withoutSelection: true } ) )
				.toEqual( '<paragraph><$text subscript="true">foo</$text>bar</paragraph>' );

			expect( editor.getData() ).toEqual( '<p><sub>foo</sub>bar</p>' );
		} );
	...
	describe( 'editing pipeline conversion', () => {
		it( 'should convert attribute', () => {
			...
			expect( _getViewData( editor.editing.view, { withoutSelection: true } ) ).toEqual( '<p><sub>foo</sub>bar</p>' );
		} );
	} );
} );
