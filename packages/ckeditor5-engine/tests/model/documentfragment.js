...
describe( 'DocumentFragment', () => {
	describe( 'constructor()', () => {
		...
		it( 'should create document fragment with children', () => {
			...
			expect( frag.childCount ).toBe( 3 );
			expect( frag.maxOffset ).toBe( 5 );

			expect( frag.getChild( 0 ) ).toHaveProperty( 'data', 'xx' );
			expect( frag.getChild( 1 ) ).toHaveProperty( 'name', 'p' );
			expect( frag.getChild( 2 ) ).toHaveProperty( 'data', 'yy' );
		} );

		it( 'should have markers list', () => {
			const frag = new ModelDocumentFragment();

			expect( frag.markers ).toBeInstanceOf( Map );
		} );

		it( 'should have artificial properties', () => {
			const frag = new ModelDocumentFragment();

			expect( frag ).toHaveProperty( 'root', frag );
			expect( frag ).toHaveProperty( 'parent', null );
			expect( frag ).toHaveProperty( 'nextSibling', null );
			expect( frag ).toHaveProperty( 'previousSibling', null );
			expect( frag ).toHaveProperty( 'document', null );
		} );

		it( 'should have `getAncestor` method that returns empty array', () => {
			const frag = new ModelDocumentFragment();

			expect( frag.getAncestors() ).toEqual( [] );
		} );
	} );
	...
	describe( 'iterator', () => {
		it( 'should iterate over document fragment\'s children', () => {
			...
			expect( Array.from( frag ) ).toEqual( [ xx, p, yy ] );
		} );
	} );
	...
	describe( 'is()', () => {
		...
		it( 'should return true for documentFragment', () => {
			...
			expect( frag.is( 'model:documentFragment' ) ).toBe( true );
		} );
		...
		it( 'should return false for other values', () => {
			...
			expect( frag.is( 'view:documentFragment' ) ).toBe( false );
		} );
	} );
	...
	describe( 'isEmpty', () => {
		...
		it( 'should return false if document fragment has children', () => {
			...
			expect( frag.isEmpty ).toBe( false );
		} );
	} );
	...
	describe( 'offsetToIndex', () => {
		...
		it( 'should return index of a node that occupies given offset in this element', () => {
			expect( frag.offsetToIndex( 0 ) ).toBe( 0 );
			expect( frag.offsetToIndex( 1 ) ).toBe( 1 );
			expect( frag.offsetToIndex( 2 ) ).toBe( 1 );
			expect( frag.offsetToIndex( 3 ) ).toBe( 1 );
			expect( frag.offsetToIndex( 4 ) ).toBe( 2 );
		} );
		...
		it( 'should return length if given offset is equal to maxOffset', () => {
			expect( frag.offsetToIndex( 5 ) ).toBe( 3 );
		} );
	} );
	...
	describe( '_insertChild', () => {
		it( 'should add children to the document fragment', () => {
			...
			expect( frag.maxOffset ).toBe( 5 );
			expect( frag.getChild( 0 ) ).toHaveProperty( 'data', 'xy' );
			expect( frag.getChild( 1 ) ).toHaveProperty( 'data', 'foo' );
		} );
		...
		it( 'should accept strings and arrays', () => {
			...
			expect( frag.maxOffset ).toBe( 3 );
			expect( frag.getChild( 0 ) ).toHaveProperty( 'data', 'abc' );

			frag._removeChildren( 0, 1 );
			frag._insertChild( 0, [ new ModelElement( 'p' ), 'abc' ] );

			expect( frag.childCount ).toBe( 2 );
			expect( frag.maxOffset ).toBe( 4 );
			expect( frag.getChild( 0 ) ).toHaveProperty( 'name', 'p' );
			expect( frag.getChild( 1 ) ).toHaveProperty( 'data', 'abc' );
		} );
		...
		it( 'should accept and correctly handle text proxies', () => {
			...
			expect( frag.childCount ).toBe( 1 );
			expect( frag.maxOffset ).toBe( 3 );
			expect( frag.getChild( 0 ) ).toBeInstanceOf( ModelText );
			expect( frag.getChild( 0 ).data ).toBe( 'cxy' );
			expect( frag.getChild( 0 ).getAttribute( 'bold' ) ).toBe( true );
		} );
	} );
	...
	describe( '_removeChildren', () => {
		it( 'should remove children from the element and return them as an array', () => {
			...
			expect( frag.childCount ).toBe( 1 );
			expect( frag.maxOffset ).toBe( 6 );

			expect( frag.getChild( 0 ) ).toHaveProperty( 'data', 'foobar' );
		...
		it( 'should remove one child when second parameter is not specified', () => {
			...
			expect( frag.getChild( 0 ).name ).toBe( 'imageBlock' );
	...
	describe( '_removeChildrenArray', () => {
		it( 'should remove children from the element', () => {
			...
			expect( frag.getChild( 0 ) ).toHaveProperty( 'data', '_1' );
			expect( frag.getChild( 1 ) ).toHaveProperty( 'data', '_5' );
			expect( frag.getChild( 2 ) ).toHaveProperty( 'data', '_6' );
		} );
	} );
	...
	describe( 'getChildIndex', () => {
		it( 'should return child index', () => {
			...
			expect( frag.getChildIndex( p ) ).toBe( 0 );
			expect( frag.getChildIndex( textBAR ) ).toBe( 1 );
			expect( frag.getChildIndex( h ) ).toBe( 2 );
		} );
	} );
	...
	describe( 'getChildStartOffset', () => {
		it( 'should return child start offset', () => {
			...
			expect( frag.getChildStartOffset( p ) ).toBe( 0 );
			expect( frag.getChildStartOffset( textBAR ) ).toBe( 1 );
			expect( frag.getChildStartOffset( h ) ).toBe( 4 );
		} );
		...
		it( 'should return null if node is not a child of that document fragment', () => {
			...
			expect( frag.getChildStartOffset( p ) ).toBe( null );
		} );
	} );
	...
	describe( 'getChildAtOffset', () => {
		it( 'should return child at given offset', () => {
			...
			expect( frag.getChildAtOffset( 0 ) ).toBe( p );
			expect( frag.getChildAtOffset( 1 ) ).toBe( textBAR );
			expect( frag.getChildAtOffset( 2 ) ).toBe( textBAR );
			expect( frag.getChildAtOffset( 3 ) ).toBe( textBAR );
			expect( frag.getChildAtOffset( 4 ) ).toBe( h );
		} );

		it( 'should return null for incorrect offset', () => {
			const frag = new ModelDocumentFragment( [ new ModelElement( 'p' ), new ModelText( 'bar' ), new ModelElement( 'h' ) ] );

			expect( frag.getChildAtOffset( -1 ) ).toBeNull();
			expect( frag.getChildAtOffset( 5 ) ).toBeNull();
		} );
	} );
	...
	describe( 'getNodeByPath', () => {
		it( 'should return the whole document fragment if path is empty', () => {
			...
			expect( frag.getNodeByPath( [] ) ).toBe( frag );
		} );
		...
		it( 'should return a descendant of this node', () => {
			...
			expect( frag.getNodeByPath( [ 0, 0, 0 ] ) ).toBe( foo );
			expect( frag.getNodeByPath( [ 0, 0, 1 ] ) ).toBe( foo );
			expect( frag.getNodeByPath( [ 0, 0, 2 ] ) ).toBe( foo );
			expect( frag.getNodeByPath( [ 0, 0, 3 ] ) ).toBe( image );
		} );
		...
		it( 'works fine with offsets', () => {
			...
			expect( frag.getNodeByPath( [ 0 ] ) ).toBe( abc );
			expect( frag.getNodeByPath( [ 1 ] ) ).toBe( abc );
			expect( frag.getNodeByPath( [ 2 ] ) ).toBe( abc );
			expect( frag.getNodeByPath( [ 3 ] ) ).toBe( paragraph );
			expect( frag.getNodeByPath( [ 3, 0 ] ) ).toBe( foo );
			expect( frag.getNodeByPath( [ 3, 1 ] ) ).toBe( foo );
			expect( frag.getNodeByPath( [ 3, 2 ] ) ).toBe( foo );
			expect( frag.getNodeByPath( [ 3, 3 ] ) ).toBe( bold );
			expect( frag.getNodeByPath( [ 3, 3, 0 ] ) ).toBe( bar );
			expect( frag.getNodeByPath( [ 3, 3, 1 ] ) ).toBe( bar );
			expect( frag.getNodeByPath( [ 3, 3, 2 ] ) ).toBe( bar );
			expect( frag.getNodeByPath( [ 3, 3, 3 ] ) ).toBe( null );
			expect( frag.getNodeByPath( [ 3, 4 ] ) ).toBe( bom );
			expect( frag.getNodeByPath( [ 3, 5 ] ) ).toBe( bom );
			expect( frag.getNodeByPath( [ 3, 6 ] ) ).toBe( bom );
			expect( frag.getNodeByPath( [ 3, 7 ] ) ).toBe( null );
			expect( frag.getNodeByPath( [ 4 ] ) ).toBe( xyz );
			expect( frag.getNodeByPath( [ 5 ] ) ).toBe( xyz );
			expect( frag.getNodeByPath( [ 6 ] ) ).toBe( xyz );
			expect( frag.getNodeByPath( [ 7 ] ) ).toBe( null );
		} );
	} );
} );
