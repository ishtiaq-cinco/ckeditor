...
function mockEmojiRepositoryValues( editor ) {
	...
	vi.spyOn( repository, 'getEmojiByQuery' );
	vi.spyOn( repository, 'getEmojiCategories' );
	vi.spyOn( repository, 'getSkinTones' );

	repository.getEmojiCategories.mockReturnValue( [
		{
	...
	repository.getSkinTones.mockReturnValue( [
...
describe( 'EmojiPicker', () => {
	let editor, editorElement, emojiPicker, emojiRepository, fetchStub;
	...
	describe( '_createButton()', () => {
		describe( 'a toolbar icon', () => {
			...
			it( 'must register the "emoji" toolbar component if emoji repository is not ready', async () => {
				const editorElement = document.createElement( 'div' );
				document.body.appendChild( editorElement );

				vi.spyOn( console, 'warn' ).mockImplementation( () => {} );
				fetchStub.mockRejectedValue( new Error( 'Failed to load CDN.' ) );
		...
		describe( 'a menu bar item', () => {
			...
			it( 'must register the "menuBar:emoji" toolbar component if emoji repository is not ready', async () => {
				const editorElement = document.createElement( 'div' );
				document.body.appendChild( editorElement );
	...
	describe( 'showUI()', () => {
		...
		beforeEach( () => {
			balloonPlugin = editor.plugins.get( 'ContextualBalloon' );
		} );
		...
		it( 'should read categories from the repository plugin when creating UI', () => {
			...
			expect( getEmojiCategories ).toHaveBeenCalledOnce();
		} );
		...
		it( 'should add the emoji UI view to the `ContextualBalloon` plugin when opens UI', () => {
			expect( balloonPlugin.visibleView ).toBe( null );

			emojiPicker.showUI();

			expect( balloonPlugin.visibleView ).toBeInstanceOf( EmojiPickerFormView );
		} );
		...
		it( 'should update the balloon position on update event', () => {
			const updatePositionSpy = vi.spyOn( balloonPlugin, 'updatePosition' );
		...
		it( 'should close the picker when focus is on the picker and escape is clicked', () => {
			...
			balloonPlugin.visibleView.element.dispatchEvent( new KeyboardEvent( 'keydown', {
		...
		describe( 'fake visual selection', () => {
			...
			describe( 'collapsed', () => {
				it( 'should not crash when hiding a fake visual selection that is not displayed', () => {
					expect( editor.model.markers.has( 'emoji-picker' ) ).toBe( false );

					expect( () => {
						emojiPicker._hideFakeVisualSelection();
					} ).not.toThrow();
				} );
	...
	describe( '_createEmojiPickerView() - lazy repository loading', () => {
		let onReadyCallback;

		beforeEach( () => {
			vi.spyOn( emojiRepository, 'onReady' ).mockImplementation( cb => {
				onReadyCallback = cb;
			} );
		...
		it( 'should register an onReady callback when the repository is not yet ready', () => {
			emojiPicker.showUI();

			expect( emojiRepository.onReady ).toHaveBeenCalledOnce();
		} );

		it( 'should not register an onReady callback when the repository is already ready', () => {
			emojiRepository.isRepositoryReady = true;

			emojiPicker.showUI();

			expect( emojiRepository.onReady ).not.toHaveBeenCalled();
		} );

		it( 'should call setCategories when onReady fires with isReady=true and categories are available', () => {
			emojiPicker.showUI();

			const setCategories = vi.spyOn( emojiPicker.emojiPickerView, 'setCategories' ).mockImplementation( () => {} );

			onReadyCallback( true );

			expect( setCategories ).toHaveBeenCalledOnce();
			expect( setCategories ).toHaveBeenCalledWith( emojiRepository.getEmojiCategories() );
		} );

		it( 'should not call setCategories when onReady fires with isReady=false', () => {
			emojiPicker.showUI();
		...
		it( 'should not call setCategories when onReady fires with isReady=true but categories array is empty', () => {
			emojiRepository.getEmojiCategories.mockReturnValue( [] );
