...
import { ViewCollection } from '@ssmckinney/ckeditor5-ui';
...
describe( 'EmojiCategoriesView', () => {
	...
	describe( 'setCategories()', () => {
		...
		it( 'replaces rendered category buttons', () => {
			emojiCategoriesView.setCategories( newCategories );

			expect( [ ...emojiCategoriesView.buttonViews ].map( buttonView => buttonView.tooltip ) ).toEqual( [
				'animals',
				'activities'
			] );
			expect( emojiCategoriesView.element.childNodes.length ).toBe( 2 );
		} );

		it( 'replaces category buttons before rendering', () => {
			const unrenderedView = new EmojiCategoriesView( locale, { emojiCategories, categoryName: 'faces' } );

			unrenderedView.setCategories( newCategories );

			expect( [ ...unrenderedView.buttonViews ].map( buttonView => buttonView.tooltip ) ).toEqual( [
				'animals',
				'activities'
			] );

			unrenderedView.destroy();
		} );
	} );
	...
	describe( '_createCategoryButton()', () => {
		...
		it( 'does not use the `[aria-labelled-by]` attribute as the button is descriptive enough', () => {
			...
			expect( categoryButton.ariaLabelledBy ).toBeUndefined();
		} );
