...
describe( 'EmojiMention', () => {
	...
	describe( 'integrations with other plugins', () => {
		...
		it( 'should not update the mention configuration when the `:` character is already used', async () => {
			...
			expect( config.dropdownLimit ).toBeUndefined();
			expect( config.itemRenderer ).toBeUndefined();
	...
	describe( '_customItemRendererFactory()', () => {
		...
		it( 'should render the "Show all emojis" item properly', () => {
			...
			expect( locale.t.mock.calls.length ).toBe( 1 );
	...
	describe( '_overrideMentionExecuteListener()', () => {
		...
		it( 'should not stop the "mention" command execution when the emoji repository is not ready', async () => {
			...
			const insertTextSpy = vi.spyOn( editor.commands.get( 'insertText' ), 'execute' );
		...
		it( 'should open the emoji picker UI when selecting the "Show all emojis" option from the list', () => {
			...
			expect( stub ).toHaveBeenCalledWith( 'raising' );
	...
	describe( '_queryEmojiCallbackFactory()', () => {
		...
		it( 'should return a hint item when a query is too short', () => {
			...
			expect( queryResult, 'query is empty' ).toEqual( [
				{ id: ':__EMOJI_HINT:' }
			] );

			queryResult = queryEmoji( 's' );

			expect( queryResult, 'query as a single character' ).toEqual( [
				{ id: ':__EMOJI_HINT:' }
			] );
		} );
		...
		it( 'should pass the specified query to the repository plugin', () => {
			...
			expect( getEmojiByQuery.mock.calls.length ).toBe( 1 );
		...
		it( 'should return emojis with the proper skin tone when it is selected in the emoji picker plugin', () => {
			...
			getEmojiByQuery.mockReturnValue( [ thumbUpItem ] );
	...
	describe( 'code block integration', () => {
		it( 'should not activate emoji autocomplete inside a code block', async () => {
			...
			expect( mentionCommand.isEnabled ).toBe( false );
