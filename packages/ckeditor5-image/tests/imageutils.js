...
describe( 'ImageUtils plugin', () => {
	...
	describe( 'getClosestSelectedImageElement()', () => {
		...
		it( 'should return null if no element is selected and the selection has no image ancestor', () => {
			...
			expect( imageUtils.getClosestSelectedImageElement( model.document.selection ) ).toBeNull();
		} );
	...
	describe( 'isImageAllowed()', () => {
		...
		it( 'should return true when the selection is directly in a root that has a non-$root element name', () => {
	...
	describe( 'insertImage()', () => {
		...
		describe( 'config.image.insert.type set to "auto"', () => {
			...
			it( 'should set image width and height', () => new Promise( ( resolve, reject ) => {
				_setModelData( model, '<paragraph>f[o]o</paragraph>' );
			...
			it( 'should not set image width and height if `setImageSizes` parameter is false', () => new Promise( ( resolve, reject ) => {
				_setModelData( model, '<paragraph>f[o]o</paragraph>' );
			...
			it( 'should not overwrite existing image width and height', () => new Promise( ( resolve, reject ) => {
				_setModelData( model, '<paragraph>f[o]o</paragraph>' );

				// Insert image without width/height so the outer guard in setImageNaturalSizeAttributes passes
				// and the img.load listener is registered (img.src is set, image starts loading).
				const imageElement = imageUtils.insertImage( { src: '/sample.png' } );

				// Set width/height synchronously before the img.load event fires.
				// This exercises the inner false-branch: the load handler sees the attributes are
				// already present and must NOT call enqueueChange to overwrite them.
				model.change( writer => {
					writer.setAttribute( 'width', 100, imageElement );
					writer.setAttribute( 'height', 50, imageElement );
				} );

				setTimeout( () => {
					try {
						expect( _getModelData( model ) ).toBe(
							'<paragraph>f[<imageInline height="50" src="/sample.png" width="100"></imageInline>]o</paragraph>'
						);
