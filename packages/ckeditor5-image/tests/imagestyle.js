...
describe( 'ImageStyle', () => {
	...
	it( 'should have `isOfficialPlugin` static flag set to `true`', () => {
		expect( ImageStyle.isOfficialPlugin ).toBe( true );
	} );

	it( 'should have `isPremiumPlugin` static flag set to `false`', () => {
		expect( ImageStyle.isPremiumPlugin ).toBe( false );
	} );
	...
	it( 'should load ImageStyleUI plugin', () => {
		expect( editor.plugins.get( ImageStyleUI ) ).toBeInstanceOf( ImageStyleUI );
	} );
} );
