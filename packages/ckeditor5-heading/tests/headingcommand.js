	...
	describe( 'value', () => {
		...
		function test( { model: modelElement } ) {
			it( `equals ${ modelElement } when collapsed selection is placed inside ${ modelElement } element`, () => {
				_setModelData( model, `<${ modelElement }>foobar</${ modelElement }>` );
			...
			it( `equals false if moved from ${ modelElement } to non-block element`, () => {
				_setModelData( model, `<${ modelElement }>[foo]</${ modelElement }><notBlock>foo</notBlock>` );
	...
	describe( 'execute()', () => {
		...
		describe( 'non-collapsed selection', () => {
			...
			it( 'converts all elements where selection is applied', () => {
				_setModelData( model, '<heading1>fo[o</heading1><heading2>bar</heading2><heading3>baz]</heading3>' );
