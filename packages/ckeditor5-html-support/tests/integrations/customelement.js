...
describe( 'CustomElementSupport', () => {
	...
	it( 'should have `isOfficialPlugin` static flag set to `true`', () => {
		expect( CustomElementSupport.isOfficialPlugin ).toBe( true );
	} );

	it( 'should have `isPremiumPlugin` static flag set to `false`', () => {
		expect( CustomElementSupport.isPremiumPlugin ).toBe( false );
	} );

	it( 'should be named', () => {
		expect( editor.plugins.has( 'CustomElementSupport' ) ).toBe( true );
	} );
	...
	it( 'should allow unknown custom element', () => {
		...
		expect( getModelDataWithAttributes( model, { withoutSelection: true, excludeAttributes } ) ).toEqual( {
	...
	it( 'should allow <template> element', () => {
		...
		expect( editor.getData() ).toBe( '<template>bar</template>' );
	} );
	...
	it( 'should render in the editing view as an unsafe element', () => {
		...
		expect( editor.editing.view.domConverter.unsafeElements ).toContain( 'custom-foo-element' );
		expect( editor.editing.view.domConverter.unsafeElements )
			.toEqual( Array.from( new Set( editor.editing.view.domConverter.unsafeElements ).values() ) );

		expect( editor.editing.view.getDomRoot().innerHTML ).toBe(
			_VIEW_INLINE_FILLER +
	...
	it( 'should render in the editing view as a pre block (whitespace handling)', () => {
		...
		expect( editor.data.htmlProcessor.domConverter.preElements ).toContain( 'custom-foo-element' );
		expect( editor.data.htmlProcessor.domConverter.preElements )
			.toEqual( Array.from( new Set( editor.data.htmlProcessor.domConverter.preElements ).values() ) );
	} );
	...
	describe( 'element position', () => {
		...
		for ( const { name, data, model: modelData } of testCases ) {
			it( `should allow element inside ${ name }`, () => {
				...
				expect( _getModelData( model, { withoutSelection: true, excludeAttributes } ) ).toBe( modelData );
	...
	describe( 'content', () => {
		...
		it( 'should preserve <`template> element content', () => {
			...
			expect( editor.getData() ).toBe( '<p>foo&nbsp;<template>this is <p>some content</p> and more of it</template> bar</p>' );
		} );
		...
		it( 'should not inject nbsp in the element content', () => {
			...
			expect( editor.getData() ).toBe( '<custom><custom2>c</custom2></custom>' );
		} );
	} );
	...
	describe( 'attributes', () => {
		it( 'should allow attributes', () => {
			...
			expect( editor.getData() ).toBe( '<custom-foo-element data-foo="foo">bar</custom-foo-element>' );
		} );
		...
		it( 'should allow attributes without `data-` prefix', () => {
			...
			expect( editor.getData() ).toBe( '<custom-foo-element foo="bar">baz</custom-foo-element>' );
		} );
		...
		it( 'should ignore attributes with invalid name', () => {
			...
			expect( editor.getData() ).toBe( '<custom-foo-element data-foo="bar">baz</custom-foo-element>' );

			expect( consoleWarnStub ).toHaveBeenCalledOnce();
			expect( consoleWarnStub.mock.calls[ 0 ][ 0 ] ).toMatch( /domconverter-invalid-attribute-detected/ );
		} );
		...
		it( 'should allow attributes (classes)', () => {
			...
			expect( editor.getData() ).toBe( '<custom-foo-element class="foo">bar</custom-foo-element>' );
		} );
		...
		it( 'should allow attributes (styles)', () => {
			...
			expect( editor.getData() ).toBe( '<custom-foo-element style="background:red;">bar</custom-foo-element>' );
		} );
		...
		it( 'should allow linking custom element', () => {
			...
			expect( editor.getData() ).toBe( '<a href="bar"><custom-foo-element>bar</custom-foo-element></a>' );
		} );
	...
	it( 'should not convert html comments as a custom element', () => {
		...
		expect( editor.getData() ).toBe( '<custom>bar</custom>' );
	} );
	...
	for ( const [ data, text ] of INVALID_ELEMENTS_TEST_DATA ) {
		it( `should not convert elements with invalid names (${ data })`, () => {
			...
			expect( _getModelData( model, { withoutSelection: true } ) ).toBe( `<paragraph>${ text }</paragraph>` );

			expect( editor.getData() ).toBe( text == '' ? '' : `<p>${ text }</p>` );
		} );
	}
